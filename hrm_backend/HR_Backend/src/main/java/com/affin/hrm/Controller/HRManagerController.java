...
public class HRManagerController {
    ...
    public ResponseEntity<ApiResponse<List<EmployeeDTO>>> getAllEmployees() {
        try {
            ...
            List<EmployeeDTO> employees = employeeService.getAllEmployeesByCompany(currentUser.getCompany().getId());
    ...
    public ResponseEntity<ApiResponse<EmployeeDTO>> getEmployee(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse.error("Failed to retrieve employee: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<EmployeeDTO>> createEmployee(@Valid @RequestBody EmployeeDTO employeeDTO) {
        try {
            ...
            if (currentUser.getCompany() == null) {
                throw new RuntimeException("Current user is not associated with a company");
            }
            EmployeeDTO created = employeeService.createEmployee(employeeDTO, currentUser.getCompany().getId());
            return ResponseEntity.ok(ApiResponse.success(created, "Employee created successfully"));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.error("Failed to create employee: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<EmployeeDTO>> updateEmployee(
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse.error("Failed to update employee: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Void>> deactivateEmployee(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse.error("Failed to deactivate employee: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<Void>> terminateEmployee(
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse.error("Failed to terminate employee: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<List<DepartmentDTO>>> getAllDepartments() {
        try {
            var currentUser = authService.getCurrentEmployee();
            List<DepartmentDTO> departments = employeeService.getDepartmentsByCompany(currentUser.getCompany().getId());
    ...
    public ResponseEntity<ApiResponse<DepartmentDTO>> createDepartment(@Valid @RequestBody DepartmentDTO departmentDTO) {
        try {
            var currentUser = authService.getCurrentEmployee();
            DepartmentDTO created = employeeService.createDepartment(departmentDTO, currentUser.getCompany().getId());
    ...
    public ResponseEntity<ApiResponse<DepartmentDTO>> updateDepartment(
        ...
        try {
            DepartmentDTO updated = employeeService.updateDepartment(id, departmentDTO);
    ...
    public ResponseEntity<ApiResponse<List<AttendanceDTO>>> getDailyAttendance(
        ...
        try {
            var currentUser = authService.getCurrentEmployee();
            List<AttendanceDTO> attendances = attendanceService.getDailyAttendance(currentUser.getCompany().getId(), date);
    ...
    public ResponseEntity<ApiResponse<List<AttendanceDTO>>> getPendingAdjustments() {
        try {
            var currentUser = authService.getCurrentEmployee();
            List<AttendanceDTO> adjustments = attendanceService.getPendingAdjustments(currentUser.getCompany().getId());
            return ResponseEntity.ok(ApiResponse.success(adjustments, "Pending adjustments retrieved"));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(ApiResponse.error("Failed to retrieve adjustments: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<AttendanceDTO>> approveAdjustment(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse.error("Failed to approve adjustment: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<AttendanceDTO>> rejectAdjustment(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse.error("Failed to reject adjustment: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<List<LeaveApplicationDTO>>> getPendingLeaves() {
        try {
            var currentUser = authService.getCurrentEmployee();
            List<LeaveApplicationDTO> leaves = leaveService.getPendingLeaves(currentUser.getCompany().getId());
    ...
    public ResponseEntity<ApiResponse<LeaveApplicationDTO>> approveLeave(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse.error("Failed to approve leave: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<LeaveApplicationDTO>> rejectLeave(
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse.error("Failed to reject leave: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<List<LeaveBalanceDTO>>> getAllLeaveBalances() {
        try {
            ...
            return ResponseEntity.badRequest().body(ApiResponse.error("Failed to retrieve balances: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<ApiResponse<DashboardStatsDTO>> getDashboardStats() {
        try {
            var currentUser = authService.getCurrentEmployee();
            DashboardStatsDTO stats = employeeService.getDashboardStats(currentUser.getCompany().getId());
